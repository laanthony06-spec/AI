# 06_Workflow

> DSP 工作流库。用于把知识库、Prompt、模板和 Review 串成稳定流程。

## 文件说明

| 文件 | 用途 |
| --- | --- |
| `DSP_Requirement_Writer_Workflow.md` | 从制造需求到需求单 / 测试 / Review 的主流程 |
| `Requirement_Workflow.md` | 早期简版需求流程 |
| `Requirement_to_TestCase_Workflow.md` | 从已确认需求单单独生成 Test Case Excel |
| `Case_Capture_Workflow.md` | 需求完成后的案例沉淀流程 |
| `Codex_Graphviz_Flowchart_Workflow.md` | Codex 提取业务逻辑、Graphviz 排版、SVG / PDF 渲染与迭代 |

## 推荐入口

处理真实需求时，优先读取：

```text
DSP_Requirement_Writer_Workflow.md
```
