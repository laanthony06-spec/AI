...
所有图均使用 Graphviz `dot` 引擎、黑白样式、从上到下布局和正交连接线。开始、结束和处理节点统一使用矩形，只有判断节点使用菱形。每条边均显式使用 `:n/:s/:w/:e` 固定锚点；主流程上下连接使用 `source:s -> target:n`，判断“是”默认从底部向下，“否”从左侧或右侧引出。侧向分支使用同层约束，直接进入矩形侧边正中心。鉴于 Graphviz 会对多条共享入线自动错开，本套图不使用多线直接汇入同一节点；改用含义明确的分支结果或并列子流程，确保渲染后箭头不偏移、不穿过文字。
